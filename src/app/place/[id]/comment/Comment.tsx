...
import type { Place, Visit } from "@prisma/client";
...
export default function Comment({
  ...
  place: Place;
  existingComment: Visit | null;
}) {
  const [score, setScore] = useState(existingComment?.score ?? 3);
  const [submitted, setSubmitted] = useState(!!existingComment);
  const [comment, setComment] = useState(existingComment?.comment ?? "");

  return (
    <div className="min-h-screen max-w-[512px] mx-auto overflow-x-hidden">
      <div className="flex flex-col px-4 pt-4 items-start mb-[220px]">
        ...
        <SlidingRater locked={submitted} score={score} onChange={setScore} />
        <div className="mt-8">
          <span>
            {submitted
              ? "This is how you described your visit:"
              : "Write a short text about your visit (optional):"}
          </span>
          <textarea
            value={comment}
            onChange={(e) => setComment(e.target.value)}
            className="w-full h-[100px] p-2 border-2 border-black rounded-md"
            disabled={submitted}
            maxLength={500}
          />
        </div>
  ...
  function submit() {
    setSubmitted(true);
    addComment({ placeId: place.id, score, comment });
  }
}

function SlidingRater({
  score,
  onChange,
  locked,
}: {
